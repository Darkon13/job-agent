...
- `BROWSER_WORKER_TOKEN` — общий секрет backend'а и browser-worker. Воркер
  управляет настоящим Chromium с вашими авторизованными сессиями; без токена
  любой, кто дотянется до его порта, смог бы действовать от вашего имени.
  Backend подписывает им каждый RPC-вызов, воркер сверяет его при каждом
  запросе.
- `JOB_AGENT_API_TOKEN` — Bearer-токен HTTP API. Нужен, когда API или dashboard
  доступны не только с loopback (иначе любой, кто видит порт, мог бы запускать
  отклики и читать переписку), а также для CLI-команд `auth`/`review`/
  `qualification`: dashboard внедряет токен server-side и не отдаёт браузеру.
- `OPENAI_API_KEY`/`DEEPSEEK_API_KEY` — доступ к model-операторам для писем и
  ответов; без них работают детерминированные шаблоны и банк ответов.

Обязательны не все значения: `BROWSER_WORKER_TOKEN` нужен только при запуске с
профилем `browser` (анкеты, тесты, чаты, вход в HH); `JOB_AGENT_API_TOKEN` —
только для внешнего API и CLI; ключи моделей — только при `models` в конфиге.
Backend без browser-воркера и без публикации API работает и с пустыми
значениями: loopback-режим токена не требует.
