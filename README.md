...
Запустите кампанию вручную из dashboard (раздел «Задания» → `Запустить`) или
через API:
...
Перезапустите сервис. Начните с небольших лимитов и проверяйте dashboard
после каждого запуска. Если вакансия требует анкету, отклик остановится в
...
- `workflow` — прикладные сценарии (кампании, отклики, чаты, резюме);
