...
Текущая product version — `0.3.0-dev`; HTTP contract уже использует namespace
...
Заблокированный отклик можно явно освободить после исправления причины
(например, после смены resume hash или заполнения анкеты):
...
Действие очищает записанное решение и возвращает отклик в `ready`, после чего
обычный durable `application.submit` заново читает вакансию и повторяет
preflight. Повтор с тем же ключом возвращает существующую задачу; отклик в
другом статусе отклоняется. В dashboard у таких строк есть кнопка «Повторить».
