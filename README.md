...
### Реальная квалификация HH с подтверждением каждого ответа

Узкий Playwright worker открывает skill verification через сохранённое
состояние профиля. Зависимость устанавливается локально, системный браузер при
этом повторно не скачивается:

```sh
npm ci --prefix browser-worker --ignore-scripts

go run ./cmd/job-agent-qualification \
  -skill 510338 \
  -level Базовый \
  -kind theory \
...
До ввода точного `START` команда только открывает страницу метода: таймер и
попытка не создаются. После старта CLI показывает извлечённый вопрос и все
варианты. Номер ставит отметку в видимом Chrome, `r` позволяет перевыбрать, а
пустой Enter отдельно записывает выбор и нажимает «Далее». `q` завершает
локальный процесс без этого нажатия; уже запущенный серверный таймер HH при
этом не останавливается.

Первый проход формирует локальную историю подтверждённых пользователем
выборов. На следующем проходе точное совпадение fingerprint вопроса и полного
набора вариантов восстанавливает и отмечает прошлый выбор; пользователю
остаётся подтвердить «Далее» через Enter либо нажать `r` и изменить ответ.
Новый или изменившийся вопрос всегда требует ручного выбора. История относится
к определению теста, а не к профилю: `-profile secondary` переиспользует тот же
банк, но создаёт отдельную audit-сессию аккаунта.

Перед «Далее» worker повторно сверяет точный текст вопроса, полный набор
вариантов и фактически отмеченные runtime UUID. Если после единственного клика
ответ HH неоднозначен, CLI не кликает повторно: он перечитывает страницу и
продолжает, когда уже виден другой вопрос или итог. Если страница не
продвинулась, видимый Chrome остаётся доступен для проверки, а Enter выполняет
только новое чтение состояния.

Текст вопроса и полный набор вариантов накапливаются в `test_questions`, а
подтверждённый пользователем текст ответа — в append-only
`review_selections`. Runtime UUID используются только в текущей вкладке.
Импортированные учебные банки этим сценарием автоматически не применяются:
решение для каждого показанного вопроса принимает пользователь.
