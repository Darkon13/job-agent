...
координат, `period=1..30` и зависимые группы полей. Исполняются все четыре
source: `global` принимает также новые employment/work-поля, `similar_resume` и
`similar_vacancy` — только classic-набор, `related_vacancy` — лишь пагинацию.
Browser-канал умеет `global` и `similar_resume`; остальные возвращают
`Unsupported` и не маскируются под пустую выдачу.
