...
`tailoring.skills.allow_removals: true`, и служит оно лишь освобождению места
под добавленные теги вакансии. Локальный validator применяет ровно столько
удалений, сколько нужно, чтобы итоговый список помещался в `maximum` после
добавлений: лишние `remove` игнорируются, добавление без необходимости удалять
не трогает навыки, а если модель удалила слишком мало — план отклоняется.
Удалить несуществующий навык или все навыки нельзя, детерминированный fallback
...
вакансии существующие навыки ровно в объёме, нужном под добавленные. Модель не редактирует резюме: её решения
...
не является источником фактов: она перефразирует уже существующее резюме.
Вместе с текущим текстом «О себе» она получает наблюдаемый контекст резюме
(заголовок, ключевые навыки, опыт, образование, языки) и контекст вакансии
(название и `key_skills`). `resume_facts_file` не обязателен: если он задан,
личные значения из `facts.placeholders` заменяются плейсхолдерами, объявление
из контекста удаляется, а реальные значения подставляются после валидации.

Локальный validator отклоняет пустой и превышающий `maximum_runes` текст,
служебные JSON/code-fence ответы, управляющие символы, незаявленные
плейсхолдеры и числа, URL или e-mail, которых не было в контексте резюме,
фактах или вакансии. Поэтому модель не может «вспомнить» компанию или цифру,
которой нет в резюме.
...
Разрешённые пути выводятся из `resume` профиля: override —
`/resumes/{resume}/web/skills`, чтение контекста — заголовок, навыки, опыт,
образование и языки того же резюме. Policies `skills` и `about` можно включать
...
Если у профиля заданы `contacts`, модель получает их как плейсхолдеры
`{telegram}` и `{email}` и по базовой инструкции сохраняет контакты в самом
верху «О себе», а реальные значения подставляются после валидации.

`about` требует `model` (переиспользуется тот же `models[]`) и `maximum_runes`;
`resume_facts_file` не обязателен и нужен только для плейсхолдеров и
дополнительных явных фактов. `maximum_runes` ограничивает длину результата;
ориентируйтесь на лимит платформы. Без блока `tailoring` handler работает как
раньше.
