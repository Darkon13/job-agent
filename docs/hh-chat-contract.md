...
ошибки; новая активность всегда поднимает чат в это окно. Непрочитанный чат со
старой последней активностью остаётся за окном, поэтому усечённый discovery
дополнительно проходит `chats?filterUnread=true` и добавляет такие чаты в
наблюдения: иначе старый значок непрочитанного не попадал в локальный каталог и
его нельзя было снять штатным mark-read. Этот проход не выполняется, когда окно
покрыло весь каталог. Частый poll-джоб может сузить окно через `recent_pages`,
а полное окно по расписанию остаётся источником правды для старых диалогов. Полный history sync
