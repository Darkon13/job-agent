...
Docker публикует порт на IP хоста, а не на имени интерфейса. Поэтому стабильный контракт deployment — `JOB_AGENT_DASHBOARD_BIND_IP=<адрес интерфейса>`. Например, если нужный интерфейс имеет адрес `10.0.0.1`, dashboard публикуется только как `10.0.0.1:8081:8081`.

Имя интерфейса намеренно не передаётся приложению: контейнер не обязан видеть host network namespace, а `network_mode: host` ухудшил бы изоляцию и переносимость. Сопоставление «интерфейс → IP» при динамическом адресе должно делать узкое средство развёртывания перед `docker compose up`. Значение `0.0.0.0` использовать нельзя: API пока не имеет пользовательской аутентификации, а частная сеть является границей доступа только при bind на точный адрес и корректном firewall. Как трафик попадает на этот интерфейс — VPN, оверлей или маршрутизация — забота deployment-среды; приложению достаточно точного адреса.
...
Колонка «Резюме» показывает активную временную подстройку отклика: фазу saga,
добавленные и удалённые навыки и причину `recovery_required`. API остаётся
redacted: текстовые значения baseline и target не раскрываются. Завершённая
(`restored`) saga в списке не отображается. Контракт — в
[tailoring документа](application-resume-tailoring.md).
...
автоматически. Раздел метрик показывает только наблюдаемые значения (показы в поиске, просмотры, приглашения): скрытый платформой процент активности не выводится и не оценивается косвенно.
