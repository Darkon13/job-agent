...
- кампании каждые 4 часа: `target_successful: 10000` означает «не
  останавливаться по цели» — очередь набирается, а лимиты (200/день и pacing)
  лишь откладывают отправку; недосланное дожидается сброса окна;
- обслуживающие jobs: поднятие резюме и session refresh раз в 4 часа,
  синхронизация чатов каждые 10 минут, снимок активности каждые 30 минут;
