...
`job-agent-qualification` реализует этот контракт через отдельный JSON-lines
Playwright worker. До точного подтверждения `START` выполняется только чтение
карточки метода. После старта worker возвращает вопрос, видимые варианты и
runtime UUID; CLI сначала только меняет checked-state, а submit
`footer-next-button` разрешает отдельный Enter пользователя.

Внешняя кнопка «Начать тест» не создаёт попытку сразу: она открывает два
информационных экрана. Первый использует `modal-next-btn`, после него появляется
финальный `modal-start-btn`; только последний вызывает
`redirect_to_test` и запускает серверную попытку. Worker проходит эти экраны
только после явного `START` и затем допускает как текущую вкладку, так и новую
страницу на origin `assessment.hh.ru`.

Содержимое увиденного вопроса и все варианты сохраняются в локальном
`TestDefinition`, подтверждённые пользователем тексты — в append-only
`ReviewSelection`. Runtime UUID, cookies и assessment URL в устойчивый каталог
не попадают и в обычный вывод не печатаются. Внешний учебный банк не является
источником автоматического выбора для живой попытки.

Последний подтверждённый пользователем выбор восстанавливается между профилями
только при точном совпадении question fingerprint. Он может быть заранее
отмечен в DOM, но каждый `footer-next-button` остаётся отдельным человеческим
подтверждением; новый текст или набор вариантов возвращает ручной review.

Рабочими вариантами считаются только `input[name="answer"]`; декоративные
radio/checkbox не участвуют в чтении и выборе. Worker требует уникальный
непустой runtime UUID, проверяет итоговый checked-набор и допускает безопасный
повтор только для обратимого выставления отметок. После `footer-next-button`
повторный клик запрещён: при таймауте CLI выполняет read-only `capture`, сверяет
fingerprint страницы и либо продолжает с уже открытым вопросом, либо оставляет
пользователю видимый браузер для проверки.
