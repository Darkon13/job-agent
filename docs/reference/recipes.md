...
`text` понимает query-синтаксис HeadHunter, а не просто набор слов:

- `AND`, `OR`, `NOT` — логические операторы (`Golang AND (Kafka OR PostgreSQL)`);
- кавычки — точная фраза (`"Go developer"`);
- скобки — группировка;
- `excluded_text` — отдельный платформенный фильтр исключений
  (`"руководитель"`), не путать с клиентским `qualification.exclude_any`.

Проверено живьём: `text=Golang AND (Kafka OR PostgreSQL)` вместе с
resume-контекстом отдаёт именно Go-вакансии с высоким «Подходит по навыкам»,
без мусора вроде Python и Team Lead.

Клиентские фильтры (`include_any`, `exclude_any`, employer rules) применяются
уже к найденным вакансиям и работают вместе с платформенным синтаксисом:
серверный `text` сужает выдачу, клиентские правила дают точную границу и
причину решения.
...
Остальные фильтры — в [справочнике поиска](configuration/searches.md).
При смене `query` у существующего `tag` сервис сам начинает новую generation
поиска с пустым курсором, так что поднимать версию тега не обязательно.
