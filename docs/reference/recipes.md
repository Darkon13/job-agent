...
Провайдер модели и policy у профиля. Модель получает обезличенный контекст:
вместо личных данных — плейсхолдеры, реальные значения подставляются перед
отправкой. Обязателен fallback-пул писем, чтобы при сбое модели письмо всё
равно ушло.
