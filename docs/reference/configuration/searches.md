...
| `target_applications` | number | **да** | Сколько откликов рассылка планирует получить из этого поиска. |
| `fallback` | string | нет | `tag` следующего поиска, если этот не набрал цель. Цепочка раскрывается транзитивно и порядок сохраняется; циклы и неизвестные ссылки отклоняются при загрузке конфига. |
