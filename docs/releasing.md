...
`1.9.36` (tag `v1.9.36`); префикс `v` используется только в Git tag, а в JSON и
OCI label записывается чистая SemVer-строка. API имеет независимую версию
контракта `v1`, отражённую в путях `/api/v1/...`.

Известные ограничения `1.9.36`: создание резюме и `bootstrap.when:
...
1. заменить версию в `buildinfo/VERSION` (например, `1.9.36`) и синхронно
...
3. собрать image с commit SHA и RFC3339 build time, проверить версии трёх
   бинарей и endpoints;
4. создать annotated tag `v1.9.36` и запушить его: release workflow сам
