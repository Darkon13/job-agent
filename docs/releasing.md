...
`1.6.0` (tag `v1.6.0`); префикс `v` используется только в Git tag, а в JSON и
OCI label записывается чистая SemVer-строка. API имеет независимую версию
контракта `v1`, отражённую в путях `/api/v1/...`.

Известные ограничения `1.6.0`: создание резюме и `bootstrap.when:
...
1. заменить версию в `buildinfo/VERSION` (например, `1.6.0`) и синхронно
...
3. собрать image с commit SHA и RFC3339 build time, проверить версии трёх
   бинарей и endpoints;
4. создать annotated tag `v1.6.0` и запушить его: release workflow сам
