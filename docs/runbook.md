...
# на рабочей машине
make build
docker build -t job-agent:1.2.0 -f Dockerfile.binary \
  --build-arg VERSION=1.2.0 --build-arg COMMIT=$(git rev-parse --short=12 HEAD) .
docker save job-agent:1.2.0 | gzip -1 | ssh server 'gunzip | docker load'
