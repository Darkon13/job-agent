...
- Восстановление выполняйте на остановленном сервисе. После restore запустите
  `job-agent check ./deploy/config.json` и проверьте `/readyz`.
...
## HH просит капчу на отклик

Отклики со статусом `waiting_validation` и `decision_code: captcha_required`
означают, что прямой API-клиент получил от HH документ `hhcaptcha`. Обычный
retry в такой ситуации бессмысленен: платформа отклоняет запрос, пока
проверка не пройдена.

1. В dashboard нажмите «Проверка» у отклика (или выполните
   `job-agent captcha solve <application_id> --api http://127.0.0.1:18081`).
2. Сервис откроет страницу вакансии в контексте профиля и нажмёт
   «Откликнуться». Если HH показывает картинку с символами, она появится в
   панели «Проверка HH» (или в терминале в формате Kitty/Sixel/Unicode —
   выберите `--image-protocol file`, чтобы получить путь к PNG).
3. Введите ответ. При неверном ответе картинка обновляется, при верном
   отклик отправляется, сопроводительное письмо прикладывается автоматически.
4. После успеха сервис ставит обычный retry-таск: он находит уже созданный
   отклик на HH и переводит объект в `submitted`. Если браузерная страница
   остановилась в неизвестном состоянии (`review`), в панели будет скриншот
   страницы для ручного разбора.

Проверка привязана к профилю и занимает один браузерный контекст: пока она
идёт, не запускайте другие браузерные операции для этого профиля.
...
# на рабочей машине
make build
docker build -t job-agent:1.7.0 -f Dockerfile.binary \
  --build-arg VERSION=1.7.0 --build-arg COMMIT=$(git rev-parse --short=12 HEAD) .
docker save job-agent:1.7.0 | gzip -1 | ssh server 'gunzip | docker load'
...
Если сообщение повторяется дольше ~90 секунд (например, после серии быстрых
пересозданий контейнера предыдущий процесс не успел освободить lease), строку
можно снять вручную на остановленном сервисе:

```sh
docker compose stop job-agent
sqlite3 data/job-agent.db 'delete from runtime_instances;'
docker compose start job-agent
```

Lease не хранит бизнес-данные: удаление строки лишь разрешает новому процессу
сразу взять guard, не дожидаясь TTL.
