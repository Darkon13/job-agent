...
- Временная подстройка резюме умеет переписывать «О себе» под вакансию:
  `applications.tailoring.about` с обезличенным контекстом, локальной
  валидацией и fallback на текущий текст; `skills` и `about` объединяются в
  chain и могут включаться независимо.
