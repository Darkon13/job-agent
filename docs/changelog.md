...
- Вики: ссылки на `AGENTS.md` и примеры из `deploy/` больше не ведут на
  недоступные на сайте пути — они открывают файлы на GitHub. Сборка MkDocs
  переведена в `strict: true`, поэтому битые внутренние ссылки теперь валят CI.
