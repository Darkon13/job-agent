...
- README переписан под быстрый старт: возможности, три шага запуска, вход в HH,
  dry-run и переход к реальным откликам; подробный обзор вынесен в
  `docs/features.md`, добавлен индекс документации `docs/README.md`,
  quickstart поправлен и дополнен.
