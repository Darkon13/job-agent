...
- Tailoring умеет переписывать опыт: политика `tailoring.experience` (модель,
  `maximum_runes`) просит модель заново сформулировать описания мест работы под
  вакансию. Позиции, компании и даты не меняются; числа, e-mail и ссылки
  проверяются на заземлённость, при ошибке модели резюме остаётся текущим.
  По умолчанию выключено и на сервере пока не подключено.
