...
## Обезличенный контекст модели

Модель никогда не должна получать настоящие персональные данные. Значения
объявляются в файле фактов резервированным ключом `facts.placeholders`:

```json
{
  "facts": {
    "placeholders": {"name": "Иван", "surname": "Иванов", "phone": "+7..."},
    "stack": ["Go", "PostgreSQL"]
  }
}
```

Перед сборкой prompt оператор:

- подставляет `{name}`-плейсхолдеры вместо каждого объявленного значения во
  всех строках контекста (facts и вакансии);
- всегда заменяет работодателя на `{company_name}` — это зарезервированное
  имя, объявить его самому нельзя;
- удаляет само объявление `placeholders` из контекста модели.

Правила:

- имя плейсхолдера — строчные латинские буквы, цифры и `_`;
- модель может возвращать текст с объявленными плейсхолдерами; любое другое
  `{...}` считается ошибкой (`invalid_output`) и уводит подготовку в
  fallback-письмо;
- evidence-валидация выполняется по обезличенному контексту, поэтому цитаты и
  claims согласованы между собой, а числа и контакты не появляются из воздуха;
- реальные значения подставляются локально после валидации и до сохранения
  письма; в Application и provenance попадают digests обезличенного входа и
  финального текста, но не значения плейсхолдеров;
- если объявленного плейсхолдера нет, модель не должна выдумывать значение —
  она пишет фразу без него либо использует только разрешённые facts.

Так модель остаётся агностичной к личности пользователя, а единственное место
с реальными данными — локальный подстановщик перед сохранением письма.
