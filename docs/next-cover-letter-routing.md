...
Модель никогда не должна получать настоящие персональные данные. Перед
сборкой prompt контакты и личные поля заменяются плейсхолдерами, и модель
работает только с ними:

```text
{name} {surname} {middle_name} {company_name} {employer} {city} {phone} {email}
```

Правила:

- набор плейсхолдеров объявляется конфигурацией; неизвестное имя не
  подставляется молча;
- модель может возвращать текст с этими плейсхолдерами — они не считаются
  незаполненными;
- реальные значения подставляются в последний момент, перед submit, внутри
  adapter action; в Application, task payload, audit и provenance попадает
  только версия с плейсхолдерами;
- после подстановки validator повторно проверяет текст: если остался
  неизвестный или необъявленный плейсхолдер, письмо заменяется
  fallback-вариантом, а не отправляется с `{name}`;
- если тема письма или вакансии требует фактов, которых нет в
  объявленных facts, модель должна использовать плейсхолдер, а не выдумывать
  значение.

Так модель остаётся агностичной к личности пользователя, а единственное место
с реальными данными — локальный подстановщик перед отправкой.
