...
func TestConversationAPIMarksAllUnreadDialogsIdempotently(t *testing.T) {
	...
	if err := json.Unmarshal(response.Body.Bytes(), &first); err != nil || first.Matched != 1 || first.Created != 1 || len(first.Tasks) != 0 || len(queue.Tasks()) != 1 {
	...
	if err := json.Unmarshal(response.Body.Bytes(), &repeated); err != nil || repeated.Matched != 0 || repeated.Created != 0 || len(queue.Tasks()) != 1 {
