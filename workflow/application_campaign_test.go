...
func TestApplicationCampaignStopsOnDailyBudgetExhaustion(t *testing.T) {
	ctx := context.Background()
	now := time.Date(2026, 9, 23, 12, 0, 0, 0, time.UTC)
	repository := storagememory.NewRepository()
	queue := brokermemory.NewQueue()
	...
	application.FailureCategory = core.ErrorQuotaExceeded
	application.FailureMessage = "daily application budget is exhausted"
	if err := repository.SaveApplication(ctx, application, core.ApplicationPreparing); err != nil {
		t.Fatalf("save quota block: %v", err)
	}
