...
func TestDashboardServesAssetsAndProxiesAPI(t *testing.T) {
	...
	if response.Code != http.StatusOK || !strings.Contains(response.Body.String(), "Job Agent") || !strings.Contains(response.Body.String(), "Профиль и резюме") || !strings.Contains(response.Body.String(), "Активность профилей") || !strings.Contains(response.Body.String(), "Неразрешённые ошибки задач") {
	...
	if response.Code != http.StatusOK || !strings.Contains(response.Body.String(), "base_manifest_digest") || !strings.Contains(response.Body.String(), "Одноразовое изменение") || !strings.Contains(response.Body.String(), "/reconcile") || !strings.Contains(response.Body.String(), "activity_snapshots") || !strings.Contains(response.Body.String(), "/api/v1/tasks/failed") || !strings.Contains(response.Body.String(), "controlFailedTask") {
