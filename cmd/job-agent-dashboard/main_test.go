...
func TestDashboardServesAssetsAndProxiesAPI(t *testing.T) {
	...
	if response.Code != http.StatusOK || response.Header().Get("Cache-Control") != "no-store" || !strings.Contains(response.Body.String(), "base_manifest_digest") || !strings.Contains(response.Body.String(), "Одноразовое изменение") || !strings.Contains(response.Body.String(), "/reconcile") || !strings.Contains(response.Body.String(), "activity_snapshots") || !strings.Contains(response.Body.String(), "renderCampaigns") || !strings.Contains(response.Body.String(), "/api/v1/tasks/failed") || !strings.Contains(response.Body.String(), "controlFailedTask") || !strings.Contains(response.Body.String(), "/api/v1/jobs/") || !strings.Contains(response.Body.String(), "runJob") || !strings.Contains(response.Body.String(), "/api/v1/applications?limit=200") || !strings.Contains(response.Body.String(), "applicationGroup") || !strings.Contains(response.Body.String(), "Нужно участие") || !strings.Contains(response.Body.String(), "/api/v1/conversations/mark-read") || !strings.Contains(response.Body.String(), "Снять показатели активности") || strings.Contains(response.Body.String(), "response_streak") {
