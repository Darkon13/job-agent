...
func TestValidateSearchSource(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "similar resume with modern work field", query: `{"source":"similar_resume","resume":"backend","work_format":["REMOTE"]}`, wantErr: true},
		{name: "similar vacancy with modern work field", query: `{"source":"similar_vacancy","vacancy":"123","employment_form":["FULL"]}`, wantErr: true},
